...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    db.get('SELECT * FROM users WHERE email = ?', [email], async (err, row) => {
      ...
      // Insert user
      db.run(
        'INSERT INTO users (name, email, password, plain_password, phone) VALUES (?, ?, ?, ?, ?)',
        [name, email, hashedPassword, password, phone],
        function(err) {
          if (err) {
...
app.put('/api/admin/students/:id/password', authenticateToken, authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    db.run(
      'UPDATE users SET password = ?, plain_password = ? WHERE id = ? AND role = "student"',
      [hashedPassword, password, studentId],
      function(err) {
        if (err) {
